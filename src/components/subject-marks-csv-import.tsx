...
/**
 * Bulk-enters marks for the selected class from a CSV, scoped to one
 * subject+term already chosen on the page — so unlike MarksCsvImport, there's
 * no subject column to fill in. Columns: roll_no, marks, grade (optional —
 * auto-filled from the subject's configured grade boundaries when blank).
 */
export function SubjectMarksCsvImport({
  classId,
  subject,
  term,
}: {
  ...
  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        const res = await importMarksCsv(classId, term, text, subject);
        setResult(res);
        router.refresh();
      } catch (err) {
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate">
            Columns: roll_no, marks, grade (optional — auto-filled from your grading scale above if blank).
          </p>
        </div>
